    ...
    <div class="col-md-8">
        <div class="card p-4">
            ...
            <form action="<?php echo $caminhoBase; ?>/pages/saveExperiencia.php" method="post">
                ...
                <div class="mb-3">
                    <label>Período</label>
                    <input name="data_inicio" class="form-control" placeholder="Ex: 2021 - 2023" required>
                </div>
                <div class="mb-3">
        ...
        <div class="mt-4">
            ...
            <?php
            if ($curriculo_id) {
                $stmt = $conn->prepare("SELECT * FROM experiencias WHERE curriculo_id = ? ORDER BY data_inicio DESC");
                ...
                if ($rows) {
                    foreach ($rows as $r) {
                        $periodo_texto = htmlspecialchars($r['data_fim']);
                        if ($r['atual'] == 1) {
                            $periodo_texto .= " (Atual)";
                        }
                        echo "<div class='card p-2 mb-2'><strong>" . htmlspecialchars($r['cargo']) . "</strong> — " . htmlspecialchars($r['empresa']) . " <small class='text-muted'>(" . $periodo_texto . ")</small></div>";
                    }
                } else {
